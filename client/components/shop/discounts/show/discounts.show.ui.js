      ...
      m('div.row.padding-medium', [
        m('.col.small-2-3.text-left', [
          m('h1.margin-left-small', [
            m('p.head-black.text--large',  m.route().match(/create/) ? 'Create Discount' : 'Discount')
          ])
        ])
      ])
    ]);
    ...
    return m('footer.fill-width.bottom.absolute.bg-white.z-high.border-gray-light.border--top', [
      m('div.row.padding-medium', [
        m('.col.small-1-2.text-left', [
          m('button.btn.icon-trash-red.icon--center' + (Admin.components.shop.discounts.state.isDeleteProcessing() === Admin.models.discounts.data.single().id ? '.is-loading' : ''), {})
        ]),
